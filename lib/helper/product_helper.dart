...
class ProductDBHelper {
  ...
  Future<void> insertRecord() async {
    ...
    if (isInserted == false) {
      for (int i = 0; i < AppVar.allProduct.length; i++) {
                Product data =
            Product.fromData(data: AppVar.allProduct[i],);

        String query =
            "INSERT INTO $tableName($colName, $colCategory, $colDetails, $colLike, $colPrice) VALUES(?, ?, ?, ?, ?);";
        List args = [
          data.name,
          data.category,
          data.details,
